...
class Session:
    ...
    def is_point_in_image(self, samples, x, y):
        res = False
        input_width = samples.shape[3]
        input_height = samples.shape[2]
        if x and y:
            res = (x >= 0) and (x <= input_width) and (y >= 0) and (y <= input_height)
        return res

    def eval_result(self, samples, targets, preds):
        reference = samples.shape[2]
        if samples.shape[3] < reference:
            reference = samples.shape[3]
        max_dist = 0.0095 * reference

        for bs in range(targets.shape[0]):
            for kps_num in range(NumberOfKeypoints):
                x_pred = preds[bs][kps_num * 2 + 0].item()
                y_pred = preds[bs][kps_num * 2 + 1].item()
                
                x_gt = targets[bs][kps_num * 2 + 0].item()
                y_gt = targets[bs][kps_num * 2 + 1].item()
            
                if self.is_point_in_image(samples, x_pred, y_pred) and self.is_point_in_image(samples, x_gt, y_gt):
                    dx = x_pred - x_gt
                    dy = y_pred - y_gt
                    ...
                    if dst < max_dist:
                        ...
                        self.metric_fp += 1
                elif self.is_point_in_image(samples, x_pred, y_pred) and not self.is_point_in_image(samples, x_gt, y_gt):
                    self.metric_fp += 1
                elif not self.is_point_in_image(samples, x_pred, y_pred) and self.is_point_in_image(samples, x_gt, y_gt):
                    self.metric_fn += 1
                elif not self.is_point_in_image(samples, x_pred, y_pred) and not self.is_point_in_image(samples, x_gt, y_gt):
                   self.metric_tn += 1

        
    def end_epoch(self, epoch, model, train_loss, test_loss, lr, is_last):    

        eps = 1e-15
        precision = round(self.metric_tp / (self.metric_tp + self.metric_fp + eps), 5)
        accuracy = round((self.metric_tp + self.metric_tn) / (self.metric_tp + self.metric_tn + self.metric_fp + self.metric_fn + eps), 5)
        ...
        message = f"epoch {epoch:04} lr: {lr:7.5} trn: {train_loss:7.4f} evl: {test_loss:7.4f} acc: {accuracy:7.4f} prc: {precision:7.4f}"
    ...
    def plot_dataset_tagging(self):
        ...
        for split in ["train", "val", "test"]:
            ...
            for index, (image, target) in enumerate(dataset):
                ...
                image = v2.functional.to_dtype(image, torch.uint8, scale=True)
        ...
        for split in ["train", "val", "test"]:
            ...
            with torch.no_grad():
                ...
                for samples, targets in loader:
                    ...
                    samples_count += targets.shape[0]

                    self.eval_result(samples, targets, preds)

                epoch_loss = np.sqrt(loss_value / samples_count)

                eps = 1e-15
                precision = round(self.metric_tp / (self.metric_tp + self.metric_fp + eps), 5)
                accuracy = round((self.metric_tp + self.metric_tn) / (self.metric_tp + self.metric_tn + self.metric_fp + self.metric_fn + eps), 5)
            
                message = f"{split} Loss: {epoch_loss:7.4f} acc: {accuracy:7.4f} prc: {precision:7.4f}"
                logging.info(message)
    ...
    def export_weights(self, filename):
        ...
        target = os.path.join(self.session_dir, "exported.onnx")
